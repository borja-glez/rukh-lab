// Parity with the Python tokenizers (`rukh/src/rukh/tokenize/`). Same suite as the demo's
// tests/parity.test.ts, run here against this repo's copies under src/lib/chess-lm/ (refreshed
// with `pnpm sync:tokenizer`, see SYNC.md): tests/tokenizer-copies.test.ts only compares hashes
// with ../rukh-web, so a consistent-but-wrong pair of files would pass it. These tests fail
// (never skip) when the copies are missing.
