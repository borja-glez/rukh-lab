...
Las cifras de esta lección (el 95 % de cobertura, la tabla de `tokenizer-stats.json` y las salidas
pegadas de los labs) salen de la ejecución real del pipeline en la máquina de referencia; mientras
esta lección esté en `borrador` son la estimación previa y se sustituyen por las medidas cuando esa
ejecución se regenere.
...
<Exercise n="01" title="¿Cuántas partidas duran más de 200 tokens?">
  ...
  <Solution>
    ...
    el lab 1 es la que decide `max_len`: con 200 tokens se cubre alrededor del 95 % de las
    partidas, el 5 % restante aporta ese ruido y la mayoría de esos finales largos son tablas
    teóricas. Aceptamos el sesgo y lo dejamos anotado en el manifiesto.
...
de 20 plies) y por largas (más de 300). El número de ilegales debería ser minúsculo (Lichess valida
las partidas al guardarlas); si no lo es, algo está mal en la limpieza, no en los datos.
