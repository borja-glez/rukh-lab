...
1. **Longitud de <Term id="secuencia">secuencia</Term>.** Con un token por jugada, el 95 % de las
   partidas de nuestro recorte cabe en 200 tokens. Por carácter no cabe casi ninguna: la
   <Term id="contexto">ventana de contexto</Term> que necesita el modelo para ver una partida entera
   se multiplica por cuatro o cinco.
...
usaron los modelos públicos de 25-50M parámetros que tomamos de referencia.
...
redondeada a la centena, y muestrea hasta 50 000 partidas por tramo con semilla. Es el dataset
equilibrado con el que se afina el condicionamiento por Elo en M4: sin él, el modelo vería diez
veces más partidas de 1800 que de 2400 y el token `<w2400>` apenas se entrenaría.
...
Los cinco labs siguen el orden del pipeline. Los comandos son los reales; las salidas son las de la
máquina de referencia (RTX 5090), pegadas tal cual.
...
Python es una línea. Guarda este script como `labs/m1/explore.py` y ejecútalo desde la raíz de
`rukh`; cada consulta va sobre el parquet de enero convertido a UCI.
...
print(
    ...
    SELECT count(*)                            AS partidas,
           ...
           max(n_plies)                        AS max,
           round(100.0 * avg(n_plies <= 195), 1) AS pct_cabe_en_200
    FROM read_parquet('{GAMES}')
    """)
)
...
print(
    ...
    SELECT eco, count(*) AS partidas,
           round(100.0 * avg(result = '1-0'), 1) AS pct_blancas
    FROM read_parquet('{GAMES}')
    GROUP BY eco
...
{/* OUTPUT: uv run python labs/m1/explore.py */}

Tres cosas que mirar en esa salida. La distribución de Elo empieza en 1800 por construcción (es el
filtro) y cae rápido: hay muchos más 1800 que 2200, y eso es lo que justifica `elo-bins` en M4. La
duración tiene una cola larga: la mediana anda por las 70 medias jugadas, pero el máximo pasa de
300, y el porcentaje que cabe en 195 plies (200 tokens menos los cinco de control) es la primera
estimación de cobertura del vocabulario UCI. Y las aperturas más jugadas a este nivel son las de
siempre (siciliana, española, italiana, gambito de dama), pero sus porcentajes de victoria de
blancas no son iguales: el modelo va a ver esa asimetría y la va a aprender.
...
<Exercise n="01" title="¿Cuántas partidas duran más de 200 tokens?">
  ...
  <Solution>
    ...
    GROUP BY tramo ORDER BY tramo`. La más larga se saca con `ORDER BY n_plies DESC LIMIT 1`. Al
    tokenizar, `encode_game` se queda con los primeros 199 tokens (control más 196 jugadas) y
    añade `<eos>` en la posición 200: el modelo aprende que esa partida "termina" ahí, con un
    resultado que no ve. Son menos del 5 % de las partidas y la mayoría de esos finales largos son
    posiciones de tablas teóricas; aceptamos el sesgo y lo dejamos anotado en el manifiesto.
  </Solution>
</Exercise>
...
{/* OUTPUT: uv run rukh data uci */}

Los conteos que imprime son los que van al manifiesto: filas leídas, partidas conservadas,
descartadas por jugada ilegal y descartadas por cortas (menos de 20 plies). El número de ilegales
debería ser minúsculo (Lichess valida las partidas al guardarlas); si no lo es, algo está mal en la
limpieza, no en los datos.
...
{/* OUTPUT: uv run rukh data tokenize --scheme bpe --pack */}

{/* OUTPUT: uv run rukh data tokenize --scheme san --pack */}

{/* OUTPUT: uv run rukh data tokenize --scheme uci --stats --export-fixture --pack */}
...
para que la comparación sea justa. **Tokens por partida** es la columna que decide el coste: la media dice cuánto cuesta
una época y el p95 dice qué ventana de contexto necesitas para no truncar casi nada. **% ≤ 200** es
la cobertura del bloque de 200 tokens que usa el dataloader: con UCI fijo debería estar por encima
del 95 %; con carácter, cerca de cero (una partida de 35 jugadas ya son más de 200 caracteres). Y
**vocabulario** es lo que decide el tamaño de la capa de embeddings y de la capa de salida: 2 030
frente a 35 frente a 4 096 vectores de la dimensión del modelo. Con BPE, fíjate en que la media de
tokens por partida es menor que con UCI (las fusiones de aperturas ahorran tokens) pero la varianza
es mayor.
...
{/* OUTPUT: uv run python labs/m1/loader_check.py */}

La última cifra, la fracción de `<pad>` en los objetivos de un lote, es la medida directa de lo que
se ahorra con el empaquetado en flujo: debería ser prácticamente cero. Con una partida por fila
...
<Exercise n="04" title="Mide el precio del padding">
  ...
  <Solution>
    Con la mediana de duración del lab 1 (unas 70 medias jugadas, 75 tokens con el control), una
    partida por fila deja de media unos 125 `<pad>` por ejemplo: alrededor del 60 % del lote es
    relleno y `ignore_index` lo descarta, pero la atención ya lo ha calculado. `PackedDataset` sirve
    un lote casi entero de tokens reales, así que por lote de 64 × 200 hay unas 2,5 veces más
    posiciones que contribuyen a la pérdida. Con el mismo presupuesto de GPU, el modelo ve 2,5
    veces más jugadas.
  </Solution>
</Exercise>
...
fusiones es una radiografía de la frecuencia de las aperturas en el recorte.
...
{/* OUTPUT: uv run python labs/m1/bpe_merges.py */}

Las primeras fusiones son casillas y jugadas, como predijimos en la teoría. Los tokens más largos
son trozos de apertura: si reconoces en ellos los primeros movimientos de la española o de la
siciliana, es porque son las secuencias que más se repiten en 200 000 partidas de Lichess a 1800+.
El BPE no sabe ajedrez; ha contado pares. Que contar pares baste para "descubrir" aperturas es la
intuición que necesitas para entender por qué los LLM de texto tienen tokens para `ing`, `tion` o
` the`.
...
<Exercise n="05" title="Un BPE de tamaño distinto">
  ...
  <Solution>
    ...
    jugadas y unos dos mil trozos de apertura, y la media baja por debajo del UCI fijo. La regla
...
1. **El de ejemplo**, el mate de Légal (siete jugadas). Cuenta las fichas: 13 jugadas más cinco de
   control en UCI, y compáralo con los caracteres de SAN. Cambia el Elo de blancas a 2450 y mira qué
   ficha cambia y cuál no.
...
- **Tokens por partida** por esquema (media, p50, p95) en `tokenizer-stats.json`, y el **porcentaje
  de partidas ≤ 200 tokens**: con UCI fijo, por encima del 95 %.
...
- **Paridad Python ↔ TypeScript**: la fixture de 20 partidas (`fixtures/games.json`) se reproduce
  id a id en las dos webs; `pnpm test` falla si cambia la enumeración en un lado y no en el otro, y
  un test de hash falla si las copias del tokenizador en `rukh-lab` divergen de las de `rukh-web`.
...
- **Fracción de `<pad>`** en un lote del dataloader: prácticamente cero.
