...
cuál es más fuerte. Es lo primero que se escribió del hito y de aquí sale cada fila de la lección 8.
Es también, en pequeño, cómo funcionan las arenas que ordenan LLM enfrentándolos por parejas.

Son `src/rukh/eval/match.py`, dos arreglos en el bucle de partida de M0, el subcomando de la CLI y la
parte de los tests que hace de control.

## El fichero, y la aritmética que lo justifica

El docstring hace la cuenta de la lección 1 con números.
...
Una ventaja de 50 Elo es una puntuación esperada de 0,5715, y separarla de 0,5 al 95 % cuesta
**185 partidas**. En la escalera, esos 50 Elo son 0,0712 de puntuación alrededor de 0,45, y separar
dos estimaciones así cuesta **757 partidas por lado**, 1 514 en total, antes de sumar la
irreproducibilidad de la escalera. Los dos modelos juegan la misma partida: no hay un tercero cuyo
humor haya que promediar.

Dos detalles hacen el número honesto: los colores se espejan —cada apertura se juega dos veces, una
con cada modelo de blancas— y el libro es compartido y sembrado, así que las dos partidas de un par
salen de la misma posición. Es un **diseño emparejado**: el mismo paciente prueba los dos
...
Una partida, siempre desde el punto de vista de `a`. Guardar la apertura en cada partida permite
comprobar después que las dos direcciones de un par jugaron el mismo libro.
...
Por defecto, una propuesta ilegal se rescata con un segundo sorteo restringido a jugadas legales,
que es un sorteo mejor. Así que **un modelo que propone más ilegales recibe más segundos sorteos**.
Es un examen tipo test donde cada respuesta inválida te da derecho a contestar otra vez con las
opciones malas tachadas.

Con `masked`, ningún lado tiene rescate. Sirve de control: si las dos lecturas coinciden, la
ventaja era fuerza; si no, era el rescate.
...
El veredicto, `separated`, vive junto al intervalo del que se dedujo; más abajo verás un caso en el
que la estimación se movió 22 puntos y el veredicto se dio la vuelta. Y `settings` va dentro porque
una medición sin sus ajustes no se puede comparar con otra.
...
La conversión a Elo, con su tope: un instrumento que puede devolver infinito acaba metiéndolo en
una tabla. En una barrida, lo que hay que mirar es `separated`, no el número.
...
La función que se ejecuta **antes** de pagar las partidas: ¿cuántas hacen falta para distinguir una
ventaja de `elo` de nada? El `abs(elo)` deja escrito que +50 y −50 cuestan lo mismo.
...
El libro es aleatorio y sin curar, a propósito: uno de aperturas «sanas» mediría los dos modelos
dentro del gusto de otra persona, y la pregunta es cuál juega mejor, no cuál se sabe la Ruy López.
La guarda del final descarta aperturas que terminaron antes de tiempo, y el `while` asegura el
número de aperturas pedido.
...
`_PlayerOpponent` permite que dos decoders se enfrenten. El bucle de partida conduce un lado por el
protocolo `Player`, que devuelve `(jugada, fue_legal)`, y el otro por `Opponent`, que devuelve solo
la jugada. Envolver el segundo modelo aquí hace que el enfrentamiento **reutilice el mismo bucle**
que jugó cada partida publicada, en vez de una segunda implementación que podría discrepar.

El comentario del `choose` explica el último recurso: el bucle no sabe rendirse, y un modelo que no
propone nada ya quedó contado como ilegal, así que la primera jugada legal termina la partida sin
reventar el enfrentamiento.
...
La guarda de la primera línea es la que más se agradece: un número impar de partidas se rechaza en
vez de redondearse en silencio. Un reparto de colores torcido es un sesgo que no aparece en ninguna
métrica, así que tiene que doler al escribir el comando.

El `opponent.start(...)` explícito es parte del arreglo que encontró el control: el bucle arranca
al `Player`, pero no sabe que su rival también es un modelo.
...
El <Term id="bootstrap">bootstrap</Term> se hace sobre las puntuaciones de las partidas: se
remuestrean 2 000 veces con reemplazo, cada media se convierte a Elo, y los cuantiles de esa
distribución son el intervalo. Es preguntar dos mil veces «¿y si hubieran salido otras partidas
parecidas a estas?» con las que tienes. `separated` es `low > 0 or high < 0`, la única afirmación
que admite el criterio del hito. Los `-1` y `nan` del `settings` por defecto son feos a propósito,
para que nadie los confunda con una medición.
...
El jugador del enfrentamiento es el mismo que el de la escalera; si fueran dos, estarían midiendo
dos modelos distintos con el mismo nombre.
...
El informe es Markdown generado, con tres secciones:

- **Por color**: un modelo que solo gana con un color tiene un repertorio, no una ventaja.
- **¿Son suficientes partidas?**: llama a `games_for_edge` con la ventaja que acaba de medir y, si
  no llegan, escribe que el número es una pista. Un informe que se corrige a sí mismo vale más que
  un número que hay que acordarse de cuestionar.
- **Cómo se jugó**: libro, semilla, muestreo y si las ilegales se rescataron o se enmascararon.
...
El escritor deja un `report.md` y un `results.json` con todas las partidas dentro; el agrupador de
la lección 9 necesita las puntuaciones partida a partida.

## Los dos arreglos que el control encontró

El control de la lección 1: un modelo contra una copia exacta de sí mismo tiene que dar 0,5000.
...
El primero es esta función, nueva en `p5`: le enseña una jugada a un rival si el rival sabe
recibirla. `Opponent` no tiene `observe` en su protocolo —Stockfish y el jugador aleatorio no
guardan estado—, así que un rival con estado, como un segundo modelo, iba a ciegas toda la partida.
El `getattr` con `callable` lo resuelve sin tocar el protocolo.
...
El segundo, dentro de `play_game_with`: el prompt de un jugador es la partida hasta aquí, así que un
tablero que llega con jugadas puestas se le tiene que contar. Sin este bucle, el modelo veía un
prompt vacío y un tablero con seis jugadas del libro, y proponía jugadas de la posición inicial. En
un agente o un chat es el mismo fallo: arrancar a mitad de conversación sin pasarle el historial.

<Callout kind="roto" title="999 ilegales es un regalo">
  Los dos fallos juntos produjeron un número imposible —0,975 de un modelo contra sí mismo, con 999
  propuestas ilegales— y por eso se encontraron en un minuto. Un 0,54 nadie lo habría mirado: es
  plausible para dos copias del mismo modelo con muestreo. Un control que falla estrepitosamente
  vale más que uno que falla poco, y por eso se elige uno con respuesta conocida y exacta.
</Callout>

`_tell(opponent, move)` dentro del bucle cuenta cada jugada a los dos lados a partir de ahí.
Después de los dos arreglos: **0,5000 exacto, 11 ilegales por bando**.

## El subcomando

El docstring de `rukh eval match` es documentación de uso: explica cuándo no usar la bandera que
trae.
...
La guarda del número par se repite aquí, antes de cargar nada, para que el error cueste dos segundos
en vez de los cuarenta que tarda en cargar dos modelos. El muestreo sale de la configuración de la
suite (`--config`), así que un enfrentamiento y una fila de la tabla única usan la misma
temperatura. Y la última línea impresa compara las partidas que harían falta con las que se
jugaron: con eso delante, «+40 con 100 partidas» y «+40 con 800» dejan de parecerse.

Un detalle de la ayuda: donde el docstring sugiere `games_needed_match.py <elo>`, el lab no acepta
un argumento posicional; su bandera es `--edges`, una lista separada por comas, y sin ella imprime
la tabla completa.

<Callout kind="ejecuta" title="El control: un modelo contra sí mismo">
  Con `match.py`, los dos arreglos de `game.py` y el subcomando escritos, el instrumento se estrena como
  se estrenó en el hito. El primer comando no carga nada; el segundo necesita tu `medium-v4` y la
...
El primero tiene que fallar al instante con `error: --games must be even and at least 2`. El
segundo es el control: tras los arreglos, en el hito dio `0.5000` clavado con 11 ilegales por
...
cero**. La estimación se movió 22 puntos, ruido normal con 400 partidas, y el veredicto se dio la
vuelta. Por eso las dos direcciones se corren siempre y se agrupan (lab 3).

## El test que es el control

De `tests/unit/test_match.py` van aquí los dos tests que sostienen el instrumento. El resto fija la
conversión a Elo y su tope, las 185 partidas de `games_for_edge(50)`, el rechazo del número impar,
los colores espejados sobre un libro reproducible, las ilegales por bando y el rival sin `observe`.
...
El control con un jugador de juguete: el mismo `_Scripted` en los dos lados, veinte partidas, y el
intervalo tiene que contener el cero. Con un juguete corre en milisegundos en cada guardado; con
modelos de verdad cuesta un minuto de GPU y se corre una vez.
...
Y el que fija el fallo del libro: con un tablero que llega con `e4 e5 Cf3`, los dos lados tienen
que haber recibido esas tres jugadas antes de empezar. Sin el bucle, `player.seen` empieza vacío.
...
Un instrumento para medir diferencias se construye antes que el experimento y se estrena contra una
respuesta conocida. Lo que lo hace honesto —número par de partidas, colores espejados, libro
compartido— cabe en unas pocas guardas, y la que más vale es la que se niega a hacer algo en vez de
arreglarlo en silencio. El informe lleva dentro la cuenta que dice si hay que creérselo. Todo esto
vale igual para comparar dos prompts, dos modelos o dos versiones de un agente.
