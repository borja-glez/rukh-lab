...
Cada campo es una decisión. Las que no son evidentes:
...
- **`block: int = 200`** es el contexto, y aparece en cuatro sitios más del módulo: el recorte del
  prompt, el límite de <Term id="ply">plies</Term> de una partida, la longitud que traza el exportador y los metadatos del
...
contaminado. Hay un test para eso, al final de esta lección.

## `decoder.py`: el modelo

Empezamos por la cabecera, que es donde se declara el contrato del
fichero.
...
<Callout kind="roto" title="Los nombres de los módulos son el formato de disco">
  ...
  claves de todos los checkpoints escritos hasta hoy**. Renombrar uno parece una refactorización y
  es una incompatibilidad silenciosa, porque `load_state_dict` con `strict=False` carga lo que
  reconoce y deja el resto en la inicialización. El modelo arranca, entrena y es otro. Por eso
  `rukh.train.load_state` —la lección siguiente— carga con `strict=False` y **después** comprueba a
  mano que no falta ni sobra ninguna clave salvo la que sabe que falta. Es el mismo aviso que ves al
  cargar un modelo de Hugging Face con una versión distinta de su clase («some weights were not
  initialized»): léelo siempre, porque el modelo funciona igual y ya no es el que crees.
</Callout>
...
La parte aburrida y **dos tercios de los parámetros de un bloque**. `F.gelu` en vez de
...
El bloque <Term id="pre-norm">pre-norm</Term>, con dos líneas que son el modelo entero:
...
con ganancia 1 y sesgo 0, que es lo que se quiere.
...
La inferencia: los logits del último paso y nada más. El `@torch.no_grad()` es lo
que impide que una partida de doscientos plies vaya acumulando un grafo de autograd que nadie va a
derivar.

El recorte `idx[:, -block:]` es solo una red de seguridad para que el modelo nunca lance una
excepción con una entrada larga. No sirve para jugar una partida que no cabe en el contexto,
porque se lleva por delante el `<bos>` y los dos tokens de Elo. El muestreador de «Muestrear
jugadas» recorta de otra manera, conservando la cabecera; allí se ve por qué.
...
Solo reexportes y ninguna decisión, salvo una que se nota en todo el resto del curso:
...
esta lista es lo único que hay que tocar.
...
Quince tests, y todos corren en CPU en menos de un segundo. Son la definición ejecutable de qué es
un `MoveDecoder`, y el módulo 3 los va a heredar casi enteros. Si algún día escribes o afinas otro
modelo de secuencias, de texto o de lo que sea, estos son los que merece la pena copiar: formas,
causalidad, relleno ignorado por la pérdida, determinismo con semilla y unos pasos de gradiente que
bajan la pérdida.
...
`pytest -q`: es un test que además hace de informe. Y la aserción es una horquilla generosa (38 a 45
...
Y el último, el que más tranquilidad da por línea escrita: diez pasos de AdamW sobre un lote
...
<Exercise n="01" title="Rompe el atado y mira qué test se entera">
  ...
  <Solution>
    ...
    igual. Es el mejor ejemplo del módulo de por qué un test de identidad vale la pena:
...
decoder con una atención fusionada, una inicialización que tiene en cuenta cuántas veces
