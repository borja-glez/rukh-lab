...
El primero imprime `1500.0`: una victoria y una derrota contra 1500 es rendir al 50 %. El segundo,
`1691`, que es `1500 + 400·log10(3)`: puntuar 0,75 es ganar tres por cada una que pierdes. El
tercero, `wins None`: todas ganadas es una tirada separada; una tabla ya es un contraejemplo.
...
_upsert_ no duplica filas y que la celda de Elo dice cuál de las tres situaciones es. El segundo
comando juega dos partidas cortas contra Stockfish y comprueba que la segunda vez salen de la caché
(sin Stockfish instalado, falla). El último tiene que fallar antes de cargar nada, con
