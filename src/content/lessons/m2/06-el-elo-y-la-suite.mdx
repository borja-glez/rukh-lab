...
`artifacts/web/results.json` es **la tabla única** del curso: una fila por etapa, desde la línea base
...
La corrección es cambiar cuatro números de `DEFAULT_RUNGS`, y no está en `p2`: llegó con la etiqueta
`p3-elo-1200`, que es la que M3 usa. El bloque de arriba es el de `p2` **a propósito**, porque el
...
Cuatro ficheros, 823 líneas, y no se pegan aquí: son tests de ensamblaje —que la suite se salte lo
que falta, que el informe imprima lo que hay, que un puzle se resuelva entero— y su valor está en
existir más que en leerse. Los contratos que **sí** hay que leer línea a línea están en las lecciones
anteriores. Lo que garantiza cada uno:

<div class="table-wrap" tabindex="0" role="region" aria-label="Tabla con desplazamiento horizontal">

| Fichero                                                                                                         | Líneas | Qué garantiza                                                                                                                        |
| --------------------------------------------------------------------------------------------------------------- | -----: | ------------------------------------------------------------------------------------------------------------------------------------ |
| [`tests/unit/test_elo.py`](https://github.com/borja-glez/rukh/blob/p2/tests/unit/test_elo.py)                   |    280 | Que el ajuste recupera un Elo conocido, que el bootstrap da un intervalo que lo contiene, y que una tirada separada sale con su cota |
| [`tests/unit/test_eval_suite.py`](https://github.com/borja-glez/rukh/blob/p2/tests/unit/test_eval_suite.py)     |    181 | Que un parquet ausente, unos puzles ausentes o un Stockfish ausente producen notas y no excepciones                                  |
| [`tests/unit/test_report.py`](https://github.com/borja-glez/rukh/blob/p2/tests/unit/test_report.py)             |    188 | Que el _upsert_ sustituye la fila de una etapa en vez de duplicarla, y que la celda de Elo dice cuál de las tres situaciones es      |
| [`tests/unit/test_eval_puzzles.py`](https://github.com/borja-glez/rukh/blob/p2/tests/unit/test_eval_puzzles.py) |    154 | Que un puzle solo cuenta resuelto con la línea entera, y que `correct` registra hasta dónde llegó                                    |
