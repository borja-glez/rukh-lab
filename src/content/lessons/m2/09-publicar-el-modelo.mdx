...
De dónde sale la receta de la tarjeta: de la corrida de MLflow, por id o por el nombre más reciente
que coincida con el directorio del checkpoint. Y el `except Exception` con su comentario: **una base
de MLflow que falta, una corrida borrada o un MLflow inalcanzable no son un error**. La tarjeta se
...
la configuración, **exportar el vocabulario**, copiar los ONNX, leer la corrida y la evaluación,
...
La receta, sacada de los parámetros de MLflow, y el `config.json` entero al final. Si la corrida no
