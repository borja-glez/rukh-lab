...
`rukh publish model`: un módulo de Python y una plantilla que convierten un checkpoint en un
...
vieja en la primera reevaluación, y una tarjeta que solo dice lo bueno es un folleto. Nada de esto
es propio del ajedrez: es lo que harías para publicar un LLM afinado, un clasificador o un modelo
de embeddings en el Hub.
...
La cuarta es la que más enseña, y es una consecuencia directa de la lección 2: **la cabeza está atada al embedding, así que `lm_head.weight` y `tokens.weight` son un tensor con dos nombres.** <Term id="safetensors">safetensors</Term> se niega a serializar eso —guarda tensores, no alias— y contar los dos inflaría el
...
La mitad de los campos de `ModelPublishConfig` son enlaces —la demo, el curso, el repositorio y los
datasets—, porque una tarjeta sin enlace a la demo y al curso no lleva a ninguna parte. La licencia
por defecto es `apache-2.0`: el código del proyecto es MIT y **los pesos son Apache-2.0**. Son dos
licencias para dos cosas distintas, y la tarjeta es la de los pesos.
...
Aquí vive el cuarto párrafo de la cabecera. `.contiguous()` porque `safetensors`
...
`StrictUndefined` es la decisión importante de este bloque: una variable que la plantilla usa
...
una, porque son lo que el módulo ha medido:
...
La licencia de los pesos, la de los datos (CC0, de Lichess) y la petición de citar a Lichess. Ojo con
la primera frase: dice que el código también es Apache-2.0, y no lo es; el repositorio `rukh` es MIT,
como declaran su `LICENSE` y su `pyproject.toml`. La plantilla se equivoca en eso y los pesos, que
es lo que la tarjeta publica, sí son Apache-2.0. La última línea es la que hace la tarjeta
rastreable: la versión de `rukh` que la generó.
