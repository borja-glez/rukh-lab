...
…y el subgrupo que cuelga `publish model` del árbol de subcomandos:
...
El primero imprime el repositorio, `stage: tiny (5,309,952 parameters)`, `mode: dry-run`,
`weights: safetensors`, la ejecución de MLflow (o `not found in MLflow`) y cuatro ficheros:
`model.safetensors`, `config.json`, `tokenizer/vocab.json` y `README.md`. El segundo tiene que decir
`False True`: la cabeza atada no está en el fichero y el embedding sí. Abre el `README.md` de
`artifacts/publish/chorcat/rukh-tiny/`: si evaluaste `tiny`, trae sus números; si no, el comando que
lo arregla.
