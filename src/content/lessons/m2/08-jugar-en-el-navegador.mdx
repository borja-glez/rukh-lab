...
La mitad de la demo que toca el modelo: 784 líneas de TypeScript en `rukh-web` que descargan el `.onnx` de Hugging Face, lo meten en una sesión de <Term id="onnx">ONNX</Term> Runtime dentro de un <Term id="web-worker">web worker</Term>, comprueban que el fichero es el que dice ser y contestan los logits de la siguiente jugada sin bloquear el tablero.
...
Once líneas de comentario que valen la lección entera, y es un error que casi todo el mundo comete.
...
Trece líneas y tres reglas. El worker **posee** la sesión: el hilo principal nunca toca ORT, así que
...
Nueve líneas y un comentario que enseña algo de gestión de memoria que no es obvio: el búfer es una
...
Ese comentario de tres líneas ahorra una tarde. El literal `new Worker(new URL('./decoder.worker.ts',
