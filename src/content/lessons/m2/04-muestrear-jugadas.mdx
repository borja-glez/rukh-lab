...
recorte», porque en la línea de comandos no se puede escribir `None`; el `finally` cierra Stockfish
...
Cada comando imprime el oponente, la línea `mask:` y una línea por partida con resultado, plies,
...
El primero tiene que dar `22 passed, 2 deselected` (los de CUDA y Stockfish). El segundo ejecuta esos
