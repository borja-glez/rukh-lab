...
exista en un repositorio cuya CI de CPU no instala `transformers`. El `# noqa: E402` del import de
...
```yaml title=".github/workflows/ml.yml"
      # `hf` as well as `cpu`: without it the Hugging Face tests skip, and one of them is the
      # only check that the hand-written LoRA of `rukh.models.lora` is the same update rule as
      # `peft`'s. A claim that is verified on one laptop is not verified.
      - name: Sync (CPU torch, Hugging Face extra, dev group)
        run: uv sync --extra cpu --extra hf --group dev
```

<Src file=".github/workflows/ml.yml" tag="p4" lines="27-31" />

Y aun así la CI **sí** lo instala, con el motivo escrito encima: sin el extra, los tests de Hugging
Face se saltan, y uno de ellos es la única comprobación de que la LoRA escrita a mano es la misma
regla de actualización que la de `peft`. La frase que lo cierra vale fuera del proyecto: **una
afirmación verificada en un solo portátil no está verificada**.
...
Los dos primeros son un minuto entre los dos. El afinado real descarga Qwen3-0.6B del Hub la primera
vez (1,5 GB) y corre 1 500 pasos; la evaluación por la misma escalera tarda unos 40 minutos, más que
la del decoder, porque cada jugada es una generación de texto.
