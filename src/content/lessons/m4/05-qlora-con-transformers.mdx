...
mismo puntero son el mismo tensor—. La pérdida es la del decoder, así que una corrida por
...
una corrida comparable con las recetas publicadas de QLoRA en vez de con una variante propia.
...
porque la corrida puede no hacer lo que se le pidió: `four_bit` es si la cuantización **se usó de
...
bf16, que es la del artículo de QLoRA. Está así para que la corrida sea comparable con las publicadas
...
`report_to=["mlflow"]` mete esta corrida en el mismo cuaderno que todas las demás del proyecto, que
...
El modelo base sale de `adapter_config.json`, no de la configuración de la corrida: el adaptador dice
...
verdad, que se reutiliza entre corridas.
...
cada corrida entrenaría al lector a no leerla.
...
Se mide con el `run.json` de la corrida —si se usó cuatro bits, cuántos parámetros eran entrenables,
