...
El docstring dice la decisión y su límite. El decoder es PyTorch a secas **a propósito**, pero el
resto del ecosistema habla `PreTrainedModel`, y escribir una envoltura es barato mientras que
reimplementar LoRA, DPO y GRPO no lo es.
...
empiezan desde los mismos números.
...
Y la tercera trampa, la que más caro sale: **aquí las etiquetas no se desplazan**. Casi todos los
modelos causales de `transformers` desplazan `labels` dentro de `forward`, porque esperan recibir la
misma secuencia como entrada y como etiqueta. El flujo empaquetado de este proyecto ya guarda `y` un
paso por delante de `x`, así que volver a desplazarlas entrenaría al modelo a predecir la jugada **de
...
<Callout kind="roto" title="Un desplazamiento de más no rompe nada, y ese es el problema">
  El modelo entrenaría. La pérdida bajaría, porque predecir dos pasos más allá también se puede
  aprender, y la curva tendría buena pinta. Lo único raro sería que el modelo jugara mal, y a esas
  alturas hay cinco explicaciones plausibles antes de que a nadie se le ocurra mirar las etiquetas.
  Es un tropiezo clásico de cualquiera que mete un modelo o un colador de datos propio en el
  `Trainer` de `transformers`, y se evita con un test que compare contra un desplazamiento hecho a
  mano.
</Callout>
...
El test de la config que no se confunde con un par encoder-decoder cabe en tres líneas y evita un
fallo que aparece lejos de su causa: la excepción sale al pedir una `GenerationConfig`, no al
definir el método.
...
solo rango que cubre toda la salida— las dos hacen lo mismo, y por eso es ahí donde la
igualdad significa algo.

El `name.replace("base_model.model.decoder.", "").replace(".base_layer", "")` es fontanería: traduce
el nombre de módulo que crea `peft` al que tiene el modelo desnudo. Es feo, pero se paga una vez en un test y ahorra escribir una capa de traducción en el código de producción.
...
>
  ...
  lado a lado — y eso es lo que aquí paga la envoltura.
</Transfer>
...
El docstring es el diseño del experimento. La lista de tres puntos es la condición para que un
número valga: mismas partidas, mismas métricas, y una sola diferencia —la representación—.

Los dos últimos párrafos son decisiones de curso además de técnicas. **Este fichero es
deliberadamente código de otros**: `transformers` para el modelo, `peft` para el adaptador, `trl`
para el bucle y `bitsandbytes` para los cuatro bits. El proyecto tiene su propia LoRA y su propio
bucle y aquí no los necesita; lo que necesita es recorrer el camino del ecosistema, que es el que vas
a usar de verdad. Es la receta estándar para afinar un LLM con QLoRA: cámbiale el nombre del modelo y
el fichero de datos y afinas un Llama o un Mistral con tus propios ejemplos.

Y **cuatro bits se intenta, no se exige**. La memoria que ahorra es la razón de existir de QLoRA, y a
0,6 B de parámetros en una tarjeta de 32 GB no hay nada que ahorrar: lo honesto es intentarlo,
informar de si funcionó y caer a bf16 si la dependencia no carga, en vez de fingir que se demostró la
técnica.
...
Es todo el análisis sintáctico, y la decisión está en el docstring: **solo se considera el primer
token**. Un modelo que escribe `e4 e5 2. Nf3` ha contestado a la pregunta y luego ha seguido;
...
`AmbiguousMoveError` frente a `InvalidMoveError` son las tres preguntas que había que
contestar.
...
cortada—, que es la parte que no puede diferir entre los dos.
...
El `run.json` contesta a la pregunta de la memoria, y a esta escala la respuesta es la que se
esperaba: poco. Cuatro bits sobre Qwen3-0.6B ahorraron **341 MB de pesos** (un 29 %) y **7 MB del
pico** (un 0,4 %). La tabla de embeddings, el 25,9 % del modelo, no se cuantiza, y el pico lo marcan
las activaciones y el optimizador, no los pesos congelados. La ventaja de QLoRA aparece cuando el
modelo no cabe, y a 0,6 B en una tarjeta de 32 GB cabe todo.

## Qué has aprendido

Cómo se entra en un ecosistema sin mudarse a él: una envoltura que deja una sola definición del paso
hacia delante en el repositorio, y que a cambio abre `peft`, `trl` y el Hub. Las tres trampas que
cuesta descubrir —las etiquetas ya desplazadas, el atributo de config que no se puede llamar
`decoder`, los pesos atados que hay que declarar— y el test que convierte «implementamos LoRA» en
