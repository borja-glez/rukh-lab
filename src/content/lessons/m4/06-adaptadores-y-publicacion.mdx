...
## Lo que el hito añadió al publicador de modelos

`publish/model.py` crece 383 líneas en `p4`, la mayoría para las cards del encoder de M3 y para el
muestreo. Estas son las cuatro que pertenecen a este módulo.
...
<Callout kind="hoy" title="Los tests de publicación que no se muestran aquí">
  `tests/unit/test_publish_adapter.py` son 291 líneas y `tests/unit/test_publish_model.py` 559. No
  van enteros en la lección porque lo que comprueban es plantilla —que tal fila aparece con tal
  formato— y leerlos entero enseña menos por línea que cualquiera de los ficheros de arriba. Los
  tres contratos que sí importan se pueden decir en tres frases, y cada uno tiene su test en ese
  fichero: una card no toma prestado el Elo del modelo base cuando el suyo no existe (dice «no
  medido»); el recuento de parámetros sale de la fórmula sobre el modelo base real y no del tamaño
  del fichero del test; y la carpeta que se sube lleva exactamente cinco ficheros, con los dos del
  navegador dentro. Están enlazados en la tabla del final de esta lección.
</Callout>
...
## Los tests del módulo que se citan por enlace

<div class="table-wrap" tabindex="0" role="region" aria-label="Tests de M4 citados por enlace">

| Fichero                              | Líneas | Qué garantiza                                                            |
| ------------------------------------ | -----: | ------------------------------------------------------------------------ |
| `tests/unit/test_publish_adapter.py` |    291 | la card de un adaptador no toma prestado ningún número del modelo base   |
| `tests/unit/test_publish_model.py`   |    559 | la tabla del barrido en la card, y el rechazo de una etapa que no cuadra |
| `tests/unit/test_export.py`          |    424 | el `parity.json` y el bundle con los tres ficheros ONNX                  |
| `tests/unit/test_export_encoder.py`  |    539 | la exportación del encoder de M3, que este hito toca de pasada           |
| `tests/unit/test_benchmarks.py`      |     84 | los tiempos de referencia de la suite no se degradan en silencio         |
| `tests/unit/test_dpo.py`             |    157 | `train/dpo.py`, que ya está en `p4` y pertenece a M5                     |

</div>

Todos están en [el repositorio en la etiqueta `p4`](https://github.com/borja-glez/rukh/tree/p4/tests/unit).
Se citan por enlace y no se pegan porque lo que comprueban es forma de plantilla y cableado de CLI:
importan mucho y enseñan poco por línea, que es exactamente el criterio con el que este curso decide
qué pega y qué enlaza.
