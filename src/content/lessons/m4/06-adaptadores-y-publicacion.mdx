...
plantillas de card y los trozos nuevos de `publish/model.py`. El hilo que lo une todo es una sola
decisión: **la corrección no se funde con los pesos**.

## Por qué el adaptador no se funde antes de exportar

Fundir un adaptador y exportar el resultado es la manera obvia de servir un estilo en el navegador, y
tira por la ventana lo único que LoRA había comprado. Un estilo son 221 MB de ONNX para `medium`; dos
estilos, 442 MB; y el fichero de 1,6 MB deja de significar nada. La alternativa es exportar el modelo
con los factores `A` y `B` como **entradas** del grafo, igual que los tokens, y darle en cada llamada
los del estilo que toque. Es lo que hacen los servidores de LLM que sirven cientos de adaptadores
sobre una misma base: no funden nada, pasan los factores aparte.
...
búfer es lo que estos no pueden ser —un búfer se hornea en el fichero, y el punto es que
no lo estén—.
...
La razón de ese cambio no se deduce leyendo el modelo. La asignación por índice
(`delta[..., start:stop] = ...`) se exporta como un nodo `ScatterND`, y onnxruntime web lo ejecuta en
la CPU aunque el resto del grafo vaya por WebGPU; una concatenación de multiplicaciones y ceros se
queda en el acelerador. Solo se descubre exportando, abriendo el grafo y mirando qué nodos aparecen:
el código que se exporta no es el código que corre, y entre los dos hay un traductor con opiniones.
...
`AdaptableDecoder.forward` es toda la idea: antes de correr el modelo, reparte las capas de `lora_a`
y `lora_b` por sus casillas. Durante el trazado eso se convierte en las conexiones del grafo; en
ejecución, en dos entradas más.
...
La exportación reutiliza todo lo de M2 (`_write_graph`, `verify_dynamic_seq`, `write_metadata`) y
solo añade dos entradas y seis claves de metadatos. Esas seis permiten que el navegador sepa qué
tiene que subir sin que nadie se lo escriba a mano en un `.ts`: `adapter_inputs`, `adapter_shape_a`,
`adapter_shape_b`, `adapter_module`, `adapter_r` y `adapter_scale`.
...
Y los dos últimos cubren dos fallos que no dan error hasta el final: un adaptador que la
cuantización deja sin efecto y una carpeta de ejecución que el exportador no sabe leer.
...
El docstring es el diagnóstico que el callout de arriba resume. `convert_float_to_float16` retipa
los **valores** del grafo y deja en paz el atributo `value` de `Constant` y `ConstantOfShape`, así
que un nodo que produce un bloque de ceros sigue produciéndolos en `float32` dentro de un grafo que
ya espera `float16`.
...
Prepara una carpeta con **cinco** ficheros y la sube. Los cinco son: `adapter.safetensors` (para
PyTorch), `adapter_config.json`, `README.md`, `web/adapter.bin` y `web/adapter.json`. Los mismos
factores dos veces, escritos por la misma función a partir de los mismos tensores, que es lo que
...
`_elo_text` maneja el caso que este módulo produjo de verdad: cuando se ganaron o se perdieron
**todas** las partidas no hay intervalo, hay una cota de una cola. Qwen perdió 160 de 160, así que
su card dice `< 807 (one-sided 95 % bound; every game lost)` y no un rango.
...
de que la configuración viaja al lado, y la del navegador, que explica por qué existe el fichero
plano y qué grafo lo espera.
...
`check_eval_matches` es una guarda contra un fallo muy fácil de cometer. `read_eval` encuentra
resultados por nombre de etapa, así que pasar el `--stage` equivocado produce una card cuyos pesos y
métricas vienen de checkpoints distintos, y nada en la salida lo dice.
