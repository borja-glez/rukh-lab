...
qué cambió el adaptador y qué costó. Lo une todo una decisión: **la corrección no se funde con los
pesos**.

## Por qué el adaptador no se funde antes de exportar

Fundir un adaptador y exportar el resultado es la manera obvia de servir un estilo, y tira por la
ventana lo que LoRA había comprado: un estilo son 221 MB de ONNX; dos, 442 MB. La alternativa es
exportar el modelo con los factores `A` y `B` como **entradas** del grafo, igual que los tokens, y
darle en cada llamada los del estilo que toque. Es lo que hacen los servidores de LLM que sirven
cientos de adaptadores sobre una misma base.
...
Las tres propiedades del docstring tienen test. **Los ceros son el modelo base**, hasta el último bit
de `float32`, así que el fichero adaptable sustituye al ordinario en vez de ser una segunda copia.
Otro adaptador cambia los logits sin tocar el fichero. Y hay <Term id="paridad">paridad</Term> con
PyTorch: el mismo adaptador por `apply_lora` y por este grafo da la misma jugada siguiente. El
fichero no reimplementa ni el exportador ni el adaptador: los compone.
...
La corrección es `(alpha/r)·B·A`, y ese factor tiene que vivir en un solo sitio. En el grafo, todo
adaptador tendría que haberse entrenado con el mismo `alpha` y el mismo `r`, sin nada contra lo que
comprobarlo. Así que vive en el **fichero**: `write_web_adapter` multiplica `B` por la escala antes de
escribirlo, y el grafo del navegador multiplica por uno.

<Callout kind="teoria" title="Un dato sin su unidad es media medida">
  «Está a 30» no dice nada hasta que sabes si son grados o metros. Cuando un dato y su
  interpretación se pueden separar, júntalos en el mismo fichero. Aquí la interpretación es el
  factor de escala, y meterlo dentro de los números hace que el búfer sea autocontenido.
</Callout>
...
`_Slot` no es un `nn.Module` a propósito: un tensor asignado a un módulo se hornearía en el fichero
como búfer, y el punto es que no lo esté. `SliceLoraFromInputs` es la aritmética de
`LoRALinear.forward` con un cambio: la corrección se arma con `cat` en vez de escribirse en una
rodaja de un tensor de ceros. La asignación por índice se exporta como un nodo `ScatterND`, que
onnxruntime web ejecuta en la CPU aunque el resto vaya por WebGPU. Solo se descubre abriendo el grafo
exportado: entre el código que escribes y el que corre hay un traductor con opiniones.
...
y `lora_b` por sus casillas. Durante el trazado eso se convierte en conexiones del grafo; en
...
La exportación reutiliza todo lo de M2 y añade dos entradas y seis claves de metadatos, con las que
el navegador sabe qué tiene que subir sin que nadie se lo escriba a mano en un `.ts`.
...
crudo, `A` primero y `B` después, sin cabecera que interpretar en el worker; cada dependencia es una
cosa más que puede fallar entre el Hub y el tablero. Las formas, el rango y la escala van en el
`.json` de al lado. `(b.cpu().float() * scale)` es la línea donde el `alpha / r` entra en el fichero.
...
La variante que lee el `adapter.safetensors` directamente evita cargar 440 MB de pesos para escribir
1,6 MB de flotantes. Fíjate en `_adapter_sort_key`: ordenar alfabéticamente pone el bloque 10 antes
del 2, y un adaptador apilado con las capas cruzadas no falla, da otro modelo.
...
### El exportador, desde la línea de comandos
...
`export_adaptable_onnx` cambia el modelo que recibe, y por eso lo que viene después vuelve a cargar
del checkpoint. La referencia de la paridad con adaptador cero es el checkpoint sin tocar.
...
`adapter_file` traduce la carpeta de una ejecución, que es lo que el lector tiene delante, al fichero
de pesos que espera `load_adapter`. Y `_adapter_parity` sostiene todo lo demás: el mismo adaptador en
PyTorch y en el grafo tiene que elegir la misma jugada en mil posiciones de validación.
...
`_trained_adapter` llena las `B` de ruido: sin eso, los tests de «el adaptador cambia algo»
compararían el modelo consigo mismo.
...
Un test por afirmación. El último, dos adaptadores distintos que dan dos respuestas distintas de un
solo fichero, es la funcionalidad entera en una aserción. Fíjate en las tolerancias: `< 1e-5` para
«es el mismo modelo» y `> 1e-3` para «es otro modelo»; solo con la primera pasaría un grafo que
ignora las entradas.
...
El camino del navegador de punta a punta comprueba la convención del `alpha/r`: el fichero plano,
alimentado a un grafo con escala uno, tiene que reproducir el modelo que aplica `alpha / r` por su
cuenta. Y los dos caminos de escritura producen los mismos bytes.
...
Los dos últimos cubren fallos que no dan error hasta el final. El de los ficheros cuantizados
comprueba la funcionalidad, no la estructura: que el adaptador siga cambiando la respuesta en fp16 y
en int8, porque nada garantiza que los conversores dejen en paz dos multiplicaciones cuyas entradas
vienen de fuera del grafo.

<Callout kind="roto" title="Un valor por defecto de la especificación que rompió el fp16">
  El grafo `float32` cargaba y el fp16, el que se sirve, se negaba: `Type Error: Type
  (tensor(float16)) of output arg (val_20) of node (node_ConstantOfShape_18) does not match expected
  type (tensor(float))`. Los rangos que nadie adapta son un `ConstantOfShape` de ceros escrito sin
  atributo `value`, porque la especificación dice que por defecto es un cero de `float32`. El
  conversor a fp16 retipa el grafo alrededor y no toca el nodo. **Un parámetro implícito es una
  dependencia como cualquier otra**, y una herramienta que reescribe tu fichero no la ve. Lo
  encontró el test de catorce segundos de arriba.
</Callout>
...
La corrección es darle al nodo, de forma explícita, el atributo del que dependía. Un `Constant`
siempre trae su `value`, y si el grafo quiere `float32` el valor por defecto ya es el correcto; de
ahí la condición del `if`.
...
La comprobación de paridad gana un `extra` para las dos entradas nuevas, con una condición: el
modelo del lado de PyTorch tiene que llevar ya cargado el mismo adaptador, o se mediría el adaptador
y no la exportación.
...
Doce tests en verde, en CPU. Exportan de verdad a ONNX y lo ejecutan, así que tardan algo más. Si
salen como saltados, falta `onnxruntime` en el entorno.
...
Un adaptador, por sí solo, no hace nada: sin el checkpoint base exacto es un taco de notas adhesivas
sin libro. Por eso su card empieza por el modelo base y el comando que lo carga, y publica lo que el
adaptador **cambió**. Publicarlo con el Elo del modelo base sería publicar el número de otro.
...
`AdapterCost` mide lo mismo en el modelo base y en el adaptado, con la misma suite y semilla: un
estilo que cambia la apertura es fácil; uno que cambia la apertura **y nada más** solo es una
afirmación si el «nada más» está medido. `AdapterEffect` lee la probabilidad de la apertura
directamente de la softmax, sin muestreo: contar aperturas de auto-partidas añadiría ruido, y un
número publicado que se mueve entre ejecuciones invita a promediarlo.
...
La carpeta que se sube lleva cinco ficheros: `adapter.safetensors`, `adapter_config.json`,
`README.md`, `web/adapter.bin` y `web/adapter.json`. Los mismos factores dos veces, escritos a partir
de los mismos tensores para que no puedan separarse.
...
La card de Qwen se construye con lo que la ejecución escribió sobre sí misma (el `run.json` y el
informe de la suite): nada está escrito a mano, porque una card que citara la receta prevista en vez
de la ejecutada sería el documento equivocado.
...
Cuando se pierden todas las partidas no hay intervalo sino una cota: la card de Qwen dice
`< 807 (one-sided 95 % bound; every game lost)`. Y aquí la carpeta de `peft` se publica tal cual: el
Hub conoce ese formato, y reescribirla al nuestro la haría inservible con las dos líneas de `peft`
que cualquier lector escribiría.
...
>
  Los adaptadores propios sí usan el formato del proyecto, porque quien los abre está usando `rukh`.
  El criterio es quién va a abrir el fichero: si es tu código, tu formato; si es el de otros, el
  suyo.
</Transfer>

## Las dos plantillas de card

Las cards se generan con Jinja a partir de la ejecución y de la evaluación. La de un adaptador va
entera porque su estructura es la política de publicación del proyecto.
...
Cuando el efecto no se ha medido, la card dice «Not measured yet» en vez de omitir la sección: una
sección ausente se lee como un descuido, y una frase que dice que no se midió es una afirmación.
...
La última frase de la sección del navegador es la tesis de la lección: «la única razón por la que
una corrección de rango bajo merece seguir siendo de rango bajo una vez sale del bucle de
entrenamiento».
...
La card de Qwen es otro documento: su `library_name` es `peft`, lo primero que dice es qué pregunta
contesta, y la fila «Precision» sale de lo que la ejecución hizo, no de lo que se le pidió.
...
Los trozos de `publish/model.py` que ponen en la card de un modelo condicionado lo que hace su
cabecera, y los que impiden publicar las métricas de otro checkpoint.
...
Un modelo cuyo punto entero es que contesta a `<wXXXX>` no se puede publicar con lo que puntúa en
una sola cabecera: lo que la cabecera hace es la afirmación, y o está medida en la card o es
publicidad. Por eso la card imprime la tabla del barrido y sus dos veredictos, que la lección 7
explica.
...
`check_eval_matches` evita un fallo muy fácil: con el `--stage` equivocado, la card tendría pesos y
métricas de checkpoints distintos y nada lo diría. Compara el `sha256` del fichero, así que rechaza
incluso dos ficheros con pesos idénticos; el falso positivo cuesta volver a evaluar, el falso
negativo cuesta una card mentirosa.
...
## Los dos subcomandos de publicación
...
`--dry-run` lo dice en la salida en vez de callarse: un comando que sube ficheros y uno que no deben
distinguirse sin recordar qué banderas se escribieron.

<Callout kind="ejecuta" title="Publicar en seco">
  Los tests de `publish/adapter.py` fijan los contratos de esta sección: una card no toma prestado
  el Elo del modelo base, el recuento de parámetros sale de la fórmula y la carpeta lleva los cinco
  ficheros. Después, un ensayo del comando sobre el Qwen de ensayo de la lección 5:
...
entradas más y un búfer plano de `float32` con la escala ya aplicada. Y cómo se publica algo que no
es un modelo: una card que empieza por lo que necesita para funcionar y mide lo que cambió y lo que
costó. Con los factores como entradas, cambiar de estilo cuesta lo que pesa el estilo, no lo que
pesa el modelo.

Lo siguiente es lo que se mide con todo esto. Sigue en «Diversidad y barridos».
