...
sin descargar otro modelo, y desde una corrida hasta un repositorio del Hub con una card que dice
...
export --adapter` toma la **carpeta** que escribió una corrida, porque es lo que el lector tiene
...
peor: un número publicado que se mueve entre corridas invita al lector a promediarlo.
...
La card de Qwen se construye de dos ficheros que la corrida escribió sobre sí misma: el `run.json`
...
Las cards no se escriben: se generan con Jinja a partir de la corrida y de la evaluación. La de un
...
«Precision» sale de `four_bit`, que es lo que la corrida **hizo**, no lo que se le pidió.
