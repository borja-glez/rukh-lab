...
El primero tiene que dar `15 passed`: son los tests del decoder de M2, sin cambiar una línea. El
segundo, la suite rápida entera en verde (algún `skipped` por marcador es normal). El tercero tiene
que salir limpio: si ruff se queja de `rope_tables` o `apply_rope` como imports sin usar, te falta
