...
Con `kind="decoder"` por defecto, la orden de M2 sigue haciendo lo mismo. La bifurcación es un
...
La orden de M2 gana `--kind`, validado a mano con código 2 (uso incorrecto) antes de tocar el disco,
...
<Callout kind="ejecuta" title="El exportador, para las dos familias">
  ...
  exporta un encoder. La segunda orden necesita un checkpoint de las cabezas (el de la tirada corta
...
<Callout kind="ejecuta" title="La exportación y la publicación, sin red">
  ...
  subir nada. La segunda orden usa el checkpoint y la carpeta ONNX de la caja anterior.
