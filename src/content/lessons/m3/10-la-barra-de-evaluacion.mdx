...
error y el segundo trabajador que las alimenta mientras el decoder elige su jugada en el primero.
...
El encoder tiene lista propia en el registro porque no es una etapa más del decoder: dentro de
...
La idea que más merece copiarse es el **encoder de juguete** de `public/test/`: una capa, ocho
...
Los dos ficheros en verde. El primero exige que los 69 tokens, el vocabulario de 47 y los ids de
las 44 posiciones del fixture coincidan uno a uno con lo que respondió Python; el segundo, entre
otras cosas, que `ENCODER_BLOCK` y `SQUARE_TOKENS` digan lo mismo.
...
Los rangos son la parte que casi nadie escribe: cada cabeza tiene el rango que su activación puede
...
Cuatro comprobaciones sobre el fichero recién descargado, porque **el registro dice lo que pedimos
y el fichero, lo que acepta**:

- dos salidas y ninguna más: un fichero con la cabeza de resultado funcionaría, pero no sería el que
  se midió;
- una sola entrada, porque `inputFeeds` entrega un tensor a `inputNames[0]` y un grafo con dos
  fallaría dentro de la inferencia con un mensaje ilegible;
- la longitud, porque `squares` no es una ventana que se pueda recortar sino 69 ranuras fijas;
- un `null` de `declaredLastDim` o `lastFixedDim` significa «no se sabe»: la dimensión simbólica se
  deja pasar y el trabajador comprueba cada secuencia.

Los mensajes acaban con la acción que resuelve el problema, «Borra los modelos descargados y vuelve
a intentarlo», porque el caso más probable es un fichero viejo en la Cache API tras una exportación
nueva.
...
`Number.isFinite(value)` sobra en rigor (un `NaN` ya falla las comparaciones), pero deja escrito
...
<Callout kind="ejecuta" title="Contrato, protocolo y cliente, sin navegador">
  Nada de esto necesita navegador: la sesión de ORT se finge y el trabajador es un doble que
  responde mensajes.
...
Los tres ficheros en verde. `contract.test.ts` prueba grafos de mentira con las salidas correctas,
con otras y con respuestas fuera de rango, y exige que solo acepte el primero; `encoder-client.test.ts`
comprueba que encoder y decoder no se cruzan las peticiones.
...
Aceptar los cuarenta megabytes del decoder no dice nada de los treinta de la barra, y la demo
funciona entera sin ella, así que la barra pide su propio consentimiento.
...
El contador de **generación** es la pieza de concurrencia del fichero. Lo habitual para descartar
...
Sin poder apagarla, el trabajador, la sesión y los treinta megabytes se quedarían toda la vida de la
...
Una inferencia que falla vacía la barra y muestra el motivo, pero la sesión sigue lista y la
...
<Transfer
  concept="Cuando «el último gana» es estructuralmente incapaz de mostrar lo que importa"
  where="Cualquier interfaz que reacciona a eventos que se pisan: un asistente que comenta lo que escribes, un validador que se dispara al cambiar un campo."
>
  Antes de escribir «gana el último», pregúntate qué eventos llegan siempre después de qué otros. Si
  una clase de evento pisa a otra por construcción, la segunda se vuelve invisible, y el síntoma es
  una función que parece no funcionar nunca. Pasa en un asistente que comenta lo que escribes (la
  última pulsación pisa el comentario de la frase anterior) o en cualquier panel del «estado actual»
  de algo que cambia más rápido de lo que se puede leer.
</Transfer>
...
De las tres reglas, la que hay que llevarse es la segunda: **la alerta nombra la jugada**. La cabeza
de errores no dice «esta posición es mala» sino «la jugada que llevó aquí tiró la partida», y una
alerta que no dice cuál es una acusación sin acusado. Cualquier clasificador que se enseña a un
usuario debe decir sobre qué se pronuncia.
...
- Dos caminos para el mismo número: el `aria-label` del `role="img"` se lee al enfocar la barra y no
  se repite cuando cambia; el `<p>` con `aria-live="polite"` anuncia el cambio.
- La región viva existe siempre, vacía si no hay nada que decir, porque **una región `aria-live`
  tiene que existir antes que el texto que anuncia**: el lector de pantalla no observa un nodo
  recién creado. Es uno de los errores de accesibilidad más comunes.
- `data-blunder`, `data-alert` y `data-raw` dejan a la suite de extremo a extremo comprobar las dos
  ramas y el número sin depender del modelo cargado, del color ni del redondeo.
...
`pnpm test` incluye `tests/evalbar.test.ts`, el de la regla de qué lectura se queda en pantalla;
`pnpm check` tiene que salir sin errores de tipos. Con `pnpm dev`, abre
`http://localhost:4321/?mock=1&encoder=test`: el rival juega la primera jugada legal y la barra usa
el encoder de juguete. Antes de aceptar, la caja de consentimiento del encoder tiene que estar a la
vista y la pestaña Red no debe mostrar ningún `.onnx`; al aceptar aparece la barra, y tras cada
jugada tuya anuncia la evaluación «tras» esa jugada (con pesos aleatorios, el número no significa
nada). Con `?mock=1&encoder=encoder-int8` descarga del Hub el encoder publicado.
...
En ningún caso vale tocar el umbral sin publicar cuál es: un punto de operación que no se declara no
se puede reproducir.
...
Servir un modelo en el navegador trae cuatro problemas que el entrenamiento no tiene, cada uno con su
técnica: un tokenizador gemelo defendido con un fixture generado desde Python; un fichero descargado
que se comprueba contra un contrato antes de dibujar nada; dos modelos que no pueden compartir cola
y van en dos trabajadores; y una barra legible sin colores, con una región viva que existe antes que
su texto. Y una lección de producto que sirve en cualquier interfaz: «gana el último» puede ser
incapaz de mostrar lo que importa, como una barra que solo acusara al rival.
