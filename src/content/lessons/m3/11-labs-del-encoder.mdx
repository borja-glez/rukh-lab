...
segunda tirada no pise los checkpoints de la primera); en los comandos se escribe `<fecha>`, y en
los tuyos va la de tu tirada (`ls -td checkpoints/encoder-mmm-*/ | head -1` te da la última). Debajo de cada comando va la salida real de la tirada de referencia, sin recortar.
...
hasta el <Term id="ply">ply</Term> _n_ y tiene que elegir el ply _n+1_ entre treinta jugadas legales, muchas de ellas
...
El último directorio de una serie sale así, y en los comandos de abajo va como
`checkpoints/encoder-heads-moves-<fecha>/` para jugadas y `checkpoints/encoder-heads-<fecha>/` para
casillas: sustituye `<fecha>` por la que te salga aquí.
...
Las dos mediciones llegaron después de cerrar el hito, y su código también: `pairwise_rank_loss`,
`HeadWeights.value_rank` y las dos configuraciones de abajo (`encoder-mmm-v4.yaml` y
`encoder-heads-v4.yaml`) son de la etiqueta `p4`, no de `p3`. En un repositorio en `p3` estas
órdenes fallan porque las configuraciones no existen; son las que se ejecutaron, desde `p4`, y
necesitan además los tokens de `data/tokens-v4/uci/` de «Más datos, no más red»:
...
anterior midió como mejor, y su `encoder_ckpt` apunta a la carpeta con fecha del preentrenamiento
de referencia (`checkpoints/encoder-mmm-v4-20260920-111628/best.pt`): cambia esa línea por la
tuya antes de la segunda orden. Si no quieres los 40 minutos, los dos checkpoints están publicados
y el `rukh pull` del clon de `main` del principio de la lección los deja en tu repositorio
(`RUKH_HOME=<ruta-a-tu-repo> uv run rukh pull encoder-mmm-v4 encoder-v4`):
`checkpoints/encoder-mmm-v4/best.pt`, el preentrenado, que es la ruta que tienes que poner en
`encoder_ckpt`, y `checkpoints/encoder-heads-v4/step-4000.pt`, el afinado, que sustituye a la
carpeta con fecha en las dos últimas órdenes.
