...
Los dos scripts y lo que necesita cada uno están además en el README de la carpeta, que es lo
primero que se lee al clonar el repositorio:

{/* prettier-ignore */}
````markdown title="labs/m3/README.md"
# M3 labs
...
<Src file="labs/m3/README.md" tag="p3" lines="1-49" />

Dos cosas de este README han quedado desfasadas y se pegan tal cual porque el curso muestra el
repositorio y no una versión mejorada de él. La primera: apunta a `curso/m3/01-el-encoder`, que
ahora es la lección de apertura y no la de labs. La segunda es más de fondo: dice que los dos
scripts son «copias de los bloques de código de la lección», y desde esta reescritura es al revés
—la lección muestra el fichero del repositorio, literal, con su enlace a GitHub—. La nota sobre la
línea partida por el límite de 100 columnas de ruff sigue siendo exacta y se ve arriba en
`shares`.
...
## Lo que el hito tocó y no está en estas lecciones

El diff de `p2` a `p3` toca sesenta y cinco ficheros y las once lecciones muestran el código del
encoder entero: los once ficheros nuevos de `src/rukh/` íntegros, los trozos cambiados de los doce
modificados que el encoder usa, las cinco configuraciones, los dos labs y la plantilla de la model
card. Lo que queda fuera queda fuera por dos motivos, y los dos se dicen aquí en vez de dejarlos
como una laguna.

**Porque es de otro módulo.** El hito arrastró trabajo que pertenece al tema de otra lección del
curso, y ahí es donde se explica:

<div class="table-wrap" tabindex="0" role="region" aria-label="Tabla con desplazamiento horizontal">

| Fichero                                                   | Qué cambió en `p3`                                                | Dónde se cuenta         |
| --------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------- |
| `src/rukh/data/puzzles.py`, `src/rukh/eval/puzzles.py`    | el prefijo de partida real de los puzles y su aviso en el informe | M2, la evaluación       |
| `src/rukh/eval/suite.py`, `src/rukh/eval/report.py`       | nombres de métrica que MLflow acepta, y la nota del prompt        | M2, la evaluación       |
| `src/rukh/data/uci.py`, `src/rukh/data/publish.py`        | retoques del pipeline de M1                                       | M1, los datos           |
| `configs/data/pipeline.yaml`, `configs/train/medium.yaml` | el corpus `v4` y el preajuste `medium`                            | M4, que es quien lo usa |
| `labs/m1/*.py`, `labs/m2/*.py`                            | la misma reconfiguración a UTF-8 de la consola de Windows         | M1 y M2, sus labs       |
| `pyproject.toml`, `README.md`, `.gitignore`               | una dependencia, el mapa del paquete y tres rutas ignoradas       | M0, el repositorio      |

</div>

**Porque es un test cuyo contrato ya está explicado.** La política del curso es enseñar enteros los
tests que enseñan un contrato y enlazar el resto diciendo qué garantiza cada uno. En este módulo van
enteros `test_encoder.py`, `test_squares.py`, `test_heuristic.py`, la mitad del enmascarado de
`test_mmm.py`, la mitad de las etiquetas de `test_labels.py` y los tests de forma, gradiente y
congelación de `test_heads.py`. Quedan enlazados, con su frase, los que necesitan media página de
andamio —un parquet de juguete, un servidor de MLflow local, una sesión de `onnxruntime`— y que
ejercitan el bucle en vez del contrato:
[`test_eval_encoder.py`](https://github.com/borja-glez/rukh/blob/p3/tests/unit/test_eval_encoder.py)
(592 líneas: la suite entera sobre una tabla de juguete, incluido que el umbral se elige en `tune` y
se mide en `score`, y que el informe imprime el aviso cuando el reparto degenera),
[`test_export_encoder.py`](https://github.com/borja-glez/rukh/blob/p3/tests/unit/test_export_encoder.py)
(459: el grafo, sus dos salidas, sus metadatos y la paridad),
[`helpers_labels.py`](https://github.com/borja-glez/rukh/blob/p3/tests/unit/helpers_labels.py)
(69: las filas de una partida real con un error de dama, que usan los dos anteriores) y la mitad de
[`test_heads.py`](https://github.com/borja-glez/rukh/blob/p3/tests/unit/test_heads.py) y de
[`test_mmm.py`](https://github.com/borja-glez/rukh/blob/p3/tests/unit/test_mmm.py) que entrena.

Y solo por enlace, por la política de siempre: `uv.lock` (generado), `tests/fixtures/` y `gold/`
(datos inertes). Leerlos no enseña nada; lo que hay que saber del `uv.lock` es lo que comprueba
`test_lock_guard.py`, que está entero en M0.
